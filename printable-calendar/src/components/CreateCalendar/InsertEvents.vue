<template>
  <div class="insertEvents">
    <v-layout v-if="!customEvents.length && !editing" align-center column> <!-- when there are none events -->
      <br />
      <span class="grayed-text">Wstaw własne wydarzenia!</span>
      <span class="grayed-text">To doskonały moment na personalizację swojego kalendarza</span>
      <br />
    ...
    <v-layout v-else-if="editing" align-center column>
      <v-col cols='6'>
        <v-form ref="form" v-model="isValid">
          ...
          <v-text-field 
            v-model="insertedEvent.text" prepend-icon="mdi-format-title" 
            label="Tytuł" :rules="titleRule"></v-text-field>
          ...
          <v-menu
            ...
            <template v-slot:activator="{ on }">
              <v-text-field
                v-on="on" label="Data"
                v-model="eventDate" prepend-icon="event"
                readonly></v-text-field>
            </template>
            <v-date-picker 
              v-model="eventDate" no-title 
              scrollable color="indigo" locale="pl" 
              :min="minCalendarDate" :max="maxCalendarDate">
            </v-date-picker>
          </v-menu>
        </v-form>
      </v-col>
    </v-layout>
    <v-row v-else> <!-- when there is some events -->
      <v-col cols="4">
        <!-- custom events -->
        <calendar-events-list 
          :events="customEvents" header="Dodane wydarzenia" 
          noEvents="Brak dodanych wydarzeń! Kliknij dodaj, by dodać wydarzenia"/>
      </v-col>
      <v-spacer />
      <v-col cols="4">
        <!-- holidays -->
        <calendar-events-list :events="holidayEvents" header="Święta" noEvents="Brak świąt!"/>
      </v-col>
    </v-row>
    ...
    <v-row justify="end" v-if="editing">
      <v-col cols="auto">
        <v-btn color="indigo" large outlined @click="editing = false">Anuluj</v-btn>
      </v-col>
      <v-col cols="auto">
        <v-btn class="white--text" color="indigo" large @click="addEvent">Dodaj</v-btn>
      </v-col>
    </v-row>
    <v-row v-else>
      <v-col cols="auto">
      ...
      <v-spacer />
      <v-col cols="auto">
        <v-btn color="indigo" large outlined @click="$emit('prev')">Wstecz</v-btn>
      </v-col>
      <v-col cols="auto">
        <v-btn class="white--text" color="indigo" large @click="$emit('next')">Dalej</v-btn>
      </v-col>
    </v-row>
  </div>
</template>
...
export default class InsertEvents extends Vue {
  ...
  isValid = true;
  editing = false;
  titleRule = [(v : any) => !!v || 'Wprowadź tytuł'];
  ...
  addEvent() {
    ...
    if (this.isValid) {
      this.$store.commit('generatedCalendar/addCustomEvent', this.insertedEvent);
