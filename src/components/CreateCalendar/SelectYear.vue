<template>
  <div class="select-year">
    <v-form ref="form" v-model="isValid">
      ...
      <v-row justify="end">
        <v-col cols="auto">
          <v-btn class="white--text" color="indigo" large @click="onClick">
            Dalej
          </v-btn>
        </v-col>
      </v-row>
...
export default class SelectYear extends Vue {
  ...
  validationRules = [(v: unknown) => !!v || "Brak wybranego roku kalendarza"];
  selectedYear = 0;
  isValid = false;
  ...
  onClick() {
    ...
    if (this.isValid) {
      ...
      if (
        !this.$store.state.generatedCalendar.calendar ||
